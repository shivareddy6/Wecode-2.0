-- ============================================================================
-- Epic 05, Story 5 revision — submissions after a round's deadline are no
-- longer rejected outright. Product decision: still let people submit
-- (LeetCode still judges it for real), just don't let it affect the
-- leaderboard, and make it visibly distinguishable from an in-contest
-- submission wherever submissions are shown (the solve screen now, chat
-- later — see docs/epics/07-chat.md's new Story 5).
--
-- is_out_of_contest is denormalized at insert time, the same way
-- difficulty already is (app/api/submissions/route.ts computes it from
-- the room's round_started_at/round_duration_seconds/round_status at the
-- moment of insert, never client-supplied) — for the same reason: a
-- round's timing is mutable/gets wholesale-replaced on the next
-- start_room_round(), so a submission can't safely re-derive after the
-- fact whether it was in-contest.
-- ============================================================================
alter table public.submissions
  add column is_out_of_contest boolean not null default false;

comment on column public.submissions.is_out_of_contest is
  'True when submitted after the room''s round deadline (or after the host ended it early) — still judged for real by LeetCode, but excluded entirely from compute_leaderboard() and flagged distinctly wherever submissions are shown.';

-- compute_leaderboard recreated with the exclusion filter. CREATE OR
-- REPLACE does NOT preserve an ALTER-set search_path — this bit us once
-- already (see the pin_compute_leaderboard_search_path migration) — so
-- `set search_path = public` is included directly in this definition
-- rather than relying on a separate ALTER FUNCTION afterward.
