...
**Status: Done — picked up for real 2026-09-23.** The three open questions from the spike framing were resolved via `AskUserQuestion` before building: **(1)** unilateral — the current host just designates someone, no accept/decline step from the new host (this is a casual friends tool, not a formal handoff; a wrong pick is one more click to undo since the outgoing host is still a participant and could transfer back). **(2)** allowed at any time, including mid-round — host powers are just single atomic writes gated on `host_user_id` (`end_round`, `start_room_round`, `close_room`, `remove_participant`), so nothing breaks if the acting host changes between one action and the next; no round-state guard needed. **(3)** transfer-only, not bundled with a "host leaves the room" flow — no story asks for generic self-removal (any participant, host or not, still can't remove their own row today), so that stays a separate, still-open gap rather than being folded in here.

`transferHost` (`apps/web/app/rooms/actions.ts`) is a direct `rooms.host_user_id` update under RLS — the same "no new SQL function" shape as `endRound`/`closeRoom`/`removeParticipant`. The RLS side needed a real change, not just a new action: the pre-existing `"hosts manage their own rooms"` policy had no explicit `WITH CHECK`, which Postgres defaults to the `USING` clause — meaning the post-update row's `host_user_id` had to still equal `auth.uid()`, so handing host off to someone else was physically blocked at the policy level regardless of what app code tried. Fixed via `supabase/migrations/20260923090000_transfer_host.sql` (`ALTER POLICY ... WITH CHECK`, applied live via the Supabase MCP): `USING` stays as-is (only the current host may touch the row at all), `WITH CHECK` is widened to also allow the new `host_user_id` to be any other active (`removed_at is null`) participant of the same room — a transfer to a non-member is rejected at the database layer, not just by app-level validation.

`ParticipantList` (`apps/web/components/participant-list.tsx`) gained a `hostUserId` prop: renders a `(Host)` tag next to the current host's row (needed so anyone — including the incoming host — can actually tell who holds the role after a transfer, not just a UI nicety) and a host-only, self-excluded "Make host" button alongside the existing "Remove" button. `apps/web/app/rooms/[code]/page.tsx` now selects `host_user_id` and passes it through. No socket broadcast on transfer — nothing in the app renders "who's host" live today; the outgoing host's own view updates via the existing `refresh()` call (same pattern as `endRound`/`closeRoom`), and the incoming host sees their new powers on their next page load/refresh.

Verified end-to-end via `npm run verify:transfer-host -w web` (9/9): non-host rejected, transfer-to-non-member rejected by the widened `WITH CHECK`, the actual transfer, the new host passing `is_room_host` and the old host failing it, the old host unable to transfer again, the new host provably holding real power (closes the room), the outgoing host's own participant row left untouched, and a bystander unaffected. Re-ran all 6 pre-existing verify scripts after — no regressions (7/7, 6/6, 6/6, 3/3, 4/4, 15/15). `tsc --noEmit` and `eslint .` both clean.

Still open, not addressed by this story: no generic "leave the room" action for any participant (host or not) — see Decision 3 above.
