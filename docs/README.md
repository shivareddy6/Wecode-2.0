...
| 06 | [Leaderboard & Scoring](./epics/06-leaderboard-scoring.md) | Time + penalty scoring, live updates, resets per session | Partial — Stories 1, 2, 3, 5 done (host competes by default, live socket push on every judged submission, per-session reset by construction, configurable constants); Story 4 (final standings) not started |
